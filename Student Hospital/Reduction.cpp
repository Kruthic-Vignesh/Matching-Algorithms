...
class StudentHospital
{
    ...
    priority_queue<pair<ll,ll>> acc_stu[2*N+1];
    map<ll,ll> stu, hos;
    vector<ll> hos_cap;
...
{
    n_stu = 0, n_hos = 0;
    stu_pref.assign(2*N+1, vector<ll>());
    hos_pref.assign(2*N+1, vector<ll>());
    pos_stu.assign(2*N+1, vector<ll>(N+1,LL_MAX));
    pos_hos.assign(2*N+1, vector<ll>(N+1,LL_MAX));
    final_list.assign(2*N+1,vector<ll>());
    allocation.assign(2*N+1,-1);
...
void StudentHospital::GetStudentList()
{
    ...
    //Adding upper level students
    for(int i=1; i<=n_stu; i++)
    {
        stu[stu[i]+offset] = i+n_stu;
        stu_rev_map.push_back(stu[i]+offset);
    }
}
